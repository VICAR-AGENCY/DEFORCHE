...
export default function DienstenGrid() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-px bg-(--color-gray-mid)">
      {diensten.map((d) => (
        <div key={d.titel} className="bg-white p-6 flex flex-col gap-3">
          ...
          {d.cta.type === "button" ? (
            <Link href="/contact" className="inline-flex items-center gap-1.5 bg-(--color-primary) text-(--color-dark) text-xs font-bold px-3 py-2 self-start hover:bg-(--color-primary-hover) transition-colors">
              <Bliksem />{d.cta.label}
            </Link>
          ) : (
      ...
      {/* Gele cel met logo */}
      <div className="bg-(--color-primary) p-6 flex flex-col items-start justify-between gap-4">
        <Image src="/logo.svg" alt="Deforche" width={110} height={36} />
        <Link href="/contact" className="inline-flex items-center gap-1.5 bg-(--color-dark) text-white text-xs font-bold px-3 py-2 hover:opacity-80 transition-opacity">
